-- Prevent transient 409 conflicts when concurrent requests initialize
-- a starter subscription for the same authenticated user.
...
begin
  ...
  if v_subscription.id is null then
    raise exception 'Unable to load or initialize active subscription for user %', p_user_id;
  end if;
